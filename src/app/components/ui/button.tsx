...
export default function Button({ children, classes, url, transparent }: Properties) {
    ...
    return (
        url?.length ? <Link href={url} className={classList} draggable={false}>{children}</Link> : <button className={classList} draggable={false}>{children}</button>
    );
}