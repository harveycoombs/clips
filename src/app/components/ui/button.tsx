...
export default function Button({ children, classes, url, transparent, ...rest }: Properties) {
    ...
    let classList = `py-2 px-4 rounded-full text-[0.8rem] leading-snug text-center font-semibold cursor-pointer select-none duration-150 ${classes?.length ? " " + classes : ""} ${appearance}`;
